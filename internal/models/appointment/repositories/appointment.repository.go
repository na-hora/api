...
func (cr *AppointmentRepository) List(companyID uuid.UUID, startDate time.Time, endDate time.Time) ([]entity.Appointment, *utils.AppError) {
	...
	data := cr.db.Where("company_id = ? AND start_time BETWEEN ? AND ?", companyID, startDate, endDate).Preload("CompanyPetServiceValue").Preload("CompanyPetServiceValue.CompanyPetService").Find(&allAppointments)
	if data.Error != nil {
